<?php
	...
	Patients pa ON ord.PatientID = pa.PatientID AND ord.clientID = pa.clientID Left JOIN SpecimenType sp ON sp.SpecimenType = ord.SpecimenType Left JOIN OrderTests ot ON ord.OrderID = ot.OrderID Left JOIN Tests t ON t.TestID = ot.TestID Left JOIN 
	Providers pr ON ord.ProviderID = pr.ProviderID Left JOIN Race ra on ra.RaceID = pa.RaceID Left JOIN Ethnicity et on et.EthnicityID = pa.EthnicityID WHERE ord.OrderID = '$flag'" ) or die("database error:". sqlsrv_error($conn));
	$testrow = array();
	while ($row = sqlsrv_fetch_array($testquery,2)) {
		// Loop through each result set and add to result array
		$testrow[] = $row;
	}
	$query = sqlsrv_query($conn, "SELECT ord.PatientID as PatientID, ord.OrderID as OrderID,* FROM Orders ord INNER JOIN 
	Patients pa ON ord.PatientID = pa.PatientID AND ProviderID = '$userid' AND ord.clientID = pa.clientID Left JOIN SpecimenType sp ON sp.SpecimenType = ord.SpecimenType Left JOIN OrderTests ot ON ord.OrderID = ot.OrderID Left JOIN Tests t ON t.TestID = ot.TestID 
		Left JOIN Providers pr ON ord.ProviderID = pr.ProviderID Left JOIN Race ra on ra.RaceID = pa.RaceID Left JOIN Ethnicity et on et.EthnicityID = pa.EthnicityID  WHERE 
	ord.OrderID = '$flag'" ) or die("database error:". sqlsrv_error($conn));
	$result = array();
							...
							<div class="col-md-10" style="border-style: dotted; border-width:2px; border-radius:10px; margin-top:10px; margin-bottom:-5px;">
								<div class="col-sm-12" style="margin-top:10px;">
									<div class="form-group">		
										<div class="alert alert-failure" id="alertform6" style="display: none;  font-size:32px; text-align:center;">
							...
							<div class="col-md-10">
								<div class="form-group row">
									...
									<div class="col-sm-7">
										...
										value="<?php if($row!=null){echo date_format($row['DateOrdered'],"Y-m-d");} else{date_format($today,"Y-m-d");}?>"
								...
								<div class="form-group row">
									...
									<div class="col-sm-7">
										<select class="form-control" id="provider">
											<option value="0">- Select -</option>
												<?php
													if (sqlsrv_num_fields($result3) > 0) {												
														do {	?>						
															<option value="<?php echo $row3['ProviderID']?>" <?php if ($row!=null && $row['ProviderID']==$row3['ProviderID']) { ?>selected<?php } ?> ><?php echo $row3['ProviderName']?></option>
					...
					<div class="row">
						...
						<div class="col-md-10">
							...
							<div class="form-group row">
								...
								<label for="dob" class="col-sm-1 col-form-label" style="text-align: right;">*</label>
								<div class="col-sm-7">
									<input type="datetime-local" class="form-control" id="cdt" maxlength="10" value="<?php if($row!=null){echo date_format($row['DateOrdered'],('Y-m-d\TH:i'));} else{date_format($today,('Y-m-d\TH:i'));}?>"
									 placeholder="Collection Date Time"  name="cdt" onkeypress='return event.charCode >= 48 && event.charCode <= 57'required>
								</div>
							</div>
							<div class="form-group row">
									...
									<option value="0">- Select -</option>
										<?php
											if (sqlsrv_num_fields($result4) > 0) {												
												do {?>						
													<option value="<?php echo $row4['SpecimenID']?>" <?php if ($row!=null && $row['SpecimenID'] == $row4['SpecimenID']) { ?>selected<?php } ?> ><?php echo $row4['SpecimenType']?></option>
									...
									<div class="col-sm-10">
										<select class="form-control" id="st1">
											...
											<?php foreach ($testtable as $table) { ?>
												...
												<?php if ($row!=null && isset($testrow[0]) && $table['TestID'] == $testrow[0]['TestID'] ) { ?>selected<?php } ?> ><?php echo $table['TestName']?></option>
											<?php } ?>
										</select>
									</div>
								</div>			
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<select class="form-control" id="st2">
											...
											<?php foreach ($testtable as $table) { ?>
												<option value="<?php echo $table['TestID']?>" 
												<?php if ($row!=null && isset($testrow[1]) && $table['TestID'] == $testrow[1]['TestID'] ) { ?>selected<?php } ?> ><?php echo $table['TestName']?></option>
											<?php } ?>
										</select>
									</div>
								</div>	
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<select class="form-control" id="st3">
											...
											<?php foreach ($testtable as $table) { ?>
												<option value="<?php echo $table['TestID']?>" 
												<?php if ($row!=null && isset($testrow[2]) && $table['TestID'] == $testrow[2]['TestID'] ) { ?>selected<?php } ?> ><?php echo $table['TestName']?></option>
											<?php } ?>
										</select>
									</div>
								</div>	
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<select class="form-control" id="st4">
											...
											<?php foreach ($testtable as $table) { ?>
												<option value="<?php echo $table['TestID']?>" 
												<?php if ($row!=null && isset($testrow[3]) && $table['TestID'] == $testrow[3]['TestID'] ) { ?>selected<?php } ?> ><?php echo $table['TestName']?></option>
											<?php } ?>
										</select>
									</div>
								</div>	
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<select class="form-control" id="st5">
											...
											<?php foreach ($testtable as $table) { ?>
												<option value="<?php echo $table['TestID']?>" 
												<?php if ($row!=null && isset($testrow[4]) && $table['TestID'] == $testrow[4]['TestID'] ) { ?>selected<?php } ?> ><?php echo $table['TestName']?></option>
											<?php } ?>
										</select>
									</div>
								</div>	
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<select class="form-control" id="st6">
											...
											<?php foreach ($testtable as $table) { ?>
												<option value="<?php echo $table['TestID']?>" 
												<?php if ($row!=null && isset($testrow[5]) && $table['TestID'] == $testrow[5]['TestID'] ) { ?>selected<?php } ?> ><?php echo $table['TestName']?></option>
											<?php } ?>
										</select>
									</div>
								</div>	
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<select class="form-control" id="st7">
											...
											<?php foreach ($testtable as $table) { ?>
												<option value="<?php echo $table['TestID']?>" 
												<?php if ($row!=null && isset($testrow[6]) && $table['TestID'] == $testrow[6]['TestID'] ) { ?>selected<?php } ?> ><?php echo $table['TestName']?></option>
											<?php } ?>
										</select>
									</div>
								</div>	
					...
					<form>
						...
						<div class="row">
							...
							<div class="col-md-10">
								...
								<div class="form-group row">
									<div class="col-md-6" >
										<button type="button" class="btn btn-primary" style="width:100%;" <?php if ($row!=null && ($row['OrderStatus'] == '10' || $row['OrderStatus'] == '20')) { ?>disabled = "disabled" <?php } ?>  onclick="saveinfo()" id="save"><i class="fa fa-save"></i>   Save</button>
									</div>
									<div class="col-md-6">
										<button type="button" class="btn btn-primary" style="width:100%;" onclick="deleteinfo()" id="delete" <?php if ($row!=null && $row['OrderStatus'] == '10' ){ ?>disabled = "disabled" <?php } ?>><i class="fa fa-trash"></i>   DELETE</button>
									</div>
								</div>			
								<div class="form-group row">
									<div class="col-md-12">
										<button type="button" class="btn btn-primary" style="width:100%;" id="send" onclick="sendlab()"  <?php if ($row!=null && $row['OrderStatus'] == '10' ){ ?>disabled = "disabled" <?php } ?>><i class = "fa fa-send"></i>   SEND to Laboratory</button>
									</div>	
								</div>
								<div class="form-group row">
									<div class="col-md-12">
										<button type="button" class="btn btn-primary" style="width:100%;" id="printRF" onclick="printDiv('printableArea')" <?php if ($row!=null && $row['OrderStatus'] != '10'){ ?>disabled = "false" <?php } ?>><i class="fa fa-print"></i>   PRINT Requisiton Form</button>
									</div>							 
								</div>			
								<div class="form-group row">
									<div class="col-md-12">
										<button type="button" class="btn btn-primary" style="width:100%;" id="printSL" onclick="printSL()" <?php if ($row!=null && $row['OrderStatus'] != '10'){ ?>disabled = "false" <?php } ?>><i class = "fa fa-print"></i>   PRINT Specimen Labels</button>
									</div>	
								</div>
								<div class="row">
									<div class="col-md-12" style="text-align: center;">
...
function showresults(){
	$.post("patientList.php",{firstname: $("#fname").val(), lastname: $("#lname").val(), mrn: $("#mrn").val(), type:1}, function(data, status){
		console.log(status);
		if(status == "success"){
			...
			{
				window.location.href = "patient_listing_provider.php?firstname="+$("#fname").val()+"&lastname="+$("#lname").val()+"&mrn="+$("#mrn").val()+"&type="+'1';
			}
...
$('#returnmenu').click(function(){
	if (<?php echo $status = 0 || $status = 10?>)
	{
		window.location.href = "menu_provider.php";
	}
...
function saveinfo(){
	...
	t_test1:t_test1,t_test2:t_test2,t_test3:t_test3,t_test4:t_test4,t_test5:t_test5,t_test6:t_test6,t_test7:t_test7,id:flag,type:1}, function(data, status){
		console.log(data);
		// alert(data);
		if(status == "success"){
			$("#alertform1").show();
...
function deleteinfo(){
	$.post("deleteresult.php",{id:"<?php echo $flag?>",type:1}, function(data, status){
		console.log(data);
		// alert(data);
		if(status == "success"){
			$("#alertform2").show();
...
function printDiv(divName) {
	// window.location.href = "print_order.php?orderid="+"<?php echo $flag ?>";
	window.open(
		"print_order.php?orderid="+"<?php echo $flag ?>",
  		'_blank' 
);
}
function printSL(){
	document.getElementById("printSL").disabled = true;
}
function sendlab(){
	$.post("sendresult.php",{id:"<?php echo $flag?>", type:1}, function(data, status){
		console.log(data);
		// alert(data);
		if(status == "success"){
			$("#alertform3").show();
