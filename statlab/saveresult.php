<?php
    ...
    if ($orderrow!=null){
        if($type == 0) {	
            ...
            $query1 = sqlsrv_query($conn, "SELECT CAST('$t_orderdate' as date) as StringToDate , CAST(GETDATE() as VARCHAR(50)) as DateToString");
            $query2 = sqlsrv_query($conn, "SELECT CAST('$t_coldate' as date) as StringToDate , CAST(GETDATE() as VARCHAR(50)) as DateToString");
            $orderdate = sqlsrv_fetch_array($query1,2);
            $time1 = $orderdate['StringToDate'];
            $coldate = sqlsrv_fetch_array($query2,2);
            $time2 = $coldate['StringToDate'];
            ...
            $query2 = sqlsrv_query($conn, "SELECT CAST('$t_coldate' as date) as StringToDate , CAST(GETDATE() as VARCHAR(50)) as DateToString");
            $orderdate = sqlsrv_fetch_array($query1,2);
            $time1 = $orderdate['DateToString'];
            $coldate = sqlsrv_fetch_array($query2,2);
            $time2 = $coldate['DateToString'];
    ...
    //This is case when the order not exist
        ...
        if ($patientrow!=null){
            if($type == 0) {
                ...
                VALUES ('$t_provider', '$clientID', '$patientID', '$t_coltech', '$t_spectype', '$time1', '$time2', '0' )");
                $sql = "INSERT INTO Orders(ProviderID, ClientID, PatientID, CollectedBy, SpecimenType, DateOrdered, DateCollected, OrderStatus) 
                VALUES ('$t_provider', '$clientID', '$patientID', '$t_coltech', '$t_spectype', '$time1', '$time2', '0' )";
        ...
        //Either patient and order not exist.
        else {         
            if($type == 0) {  
                ...
                VALUES ('$t_provider', '$userid', '$pkey', '$t_coltech', '$t_spectype', '$time1', '$time2', '0')");
                $getOrderID = sqlsrv_query($conn, "SELECT OrderID FROM Orders Order by DateIn DESC");
                $row2 = sqlsrv_fetch_array($getOrderID,2);
                $orkey = $row2['OrderID'];
                $idenon = sqlsrv_query($conn,"SET IDENTITY_INSERT OrderTests ON"); 
                ...
                var_dump("First Need to specify your client!");
                exit;
                $savepatientdata = sqlsrv_query($conn, "INSERT INTO Patients(ClientID, NameLast, NameFirst, DOB, Sex, MRN, RaceID, EthnicityID, StreetAddress, ZipCode, City, State, County, Telephone, Email)
                ...
                $pkey = $row1['PatientID'];
                $addneworderdata = sqlsrv_query($conn, "INSERT INTO Orders(ProviderID, ClientID, PatientID, CollectedBy, SpecimenType, DateOrdered, DateCollected, OrderStatus) 
                VALUES ('$t_provider', '$clientID', '$pkey', '$t_coltech', '$t_spectype', '$time1', '$time2', '0')");
                $getOrderID = sqlsrv_query($conn, "SELECT OrderID FROM Orders Order by DateIn DESC");
                $row2 = sqlsrv_fetch_array($getOrderID,2);
                $orkey = $row2['OrderID'];
                $idenon = sqlsrv_query($conn,"SET IDENTITY_INSERT OrderTests ON"); 
