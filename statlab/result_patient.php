<?php
	...
	$today = new DateTime();
	$firstDate = date_add($today,date_interval_create_from_date_string("-1 year"));
	...
	$result = sqlsrv_query($conn, "SELECT * FROM documents where daterun >= '".date_format($firstDate,"Y-m-d")."' and  daterun <= '".date_format($endDate,"Y-m-d")."' and patientID = '$userid'") or die("database error:". sqlsrv_error($conn));
	// var_dump($result,2);
?>	
<head>
...
function showresults(){
	...
	$("#pdftable").remove();
	$.post("tableList.php",{firstdate: $("#d1").val(), lastdate: $("#d2").val(), type:1}, function(data, status){
		if(status == "success"){
