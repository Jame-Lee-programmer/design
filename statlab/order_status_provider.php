<?php
	...
	if (isset($_GET)){
		...
		Patients pa ON ord.PatientID = pa.PatientID AND ord.clientID = pa.clientID AND ProviderID = '$userid' Left JOIN OrderTests ot ON ord.OrderID = ot.OrderID Left JOIN Tests t ON t.TestID = ot.TestID WHERE $new ORDER BY DateOrdered, NameFirst";
	...
	Patients pa ON ord.PatientID = pa.PatientID AND ord.clientID = pa.clientID AND ProviderID = '$userid' Left JOIN OrderTests ot ON ord.OrderID = ot.OrderID Left JOIN Tests t ON t.TestID = ot.TestID ORDER BY DateOrdered, NameFirst" ) or die("database error:". sqlsrv_error($conn));
	$result = array();
...
function returnrow(){
	...
	window.location.href = "create_order_provider.php?orderid="+id;
});
}
</script>
<script>
