<?php
    ...
    $query = sqlsrv_query($conn, "SELECT * FROM Patients WHERE NameLast like '$lname%' and NameFirst like '$fname%' and MRN like '$mrn%'") or die("database error:". sqlsrv_error($conn));			
    $result = array();
...
function returnrow(){
    window.location.href = "create_orders.php?selname="+flag;
}
function returnpage(){
    window.history.back();
}
