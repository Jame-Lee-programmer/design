...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#purchase_item').bind('keypress', function (e) {
            if (e.keyCode == 13) {
                e.preventDefault();
        ...
        $(document).on("change", '.Rqty', function () {
            ...
            purchaseItems[item_id].qty = new_qty;
            store('purchaseItems', JSON.stringify(purchaseItems));
            loadPurchaseItems();
        });
        ...
        $(document).on("change", '.Rrate', function () {
            ...
            var new_cost = $(this).val(),
                item_id = row.attr('data-item-id');
            purchaseItems[item_id].rate = new_cost;
            store('purchaseItems', JSON.stringify(purchaseItems));
            loadPurchaseItems();
        });
        ...
        $(document).on("change", '.RitemDesc', function () {
            ...
            var item_desc = $(this).val(),
                item_id = row.attr('data-item-id');
            purchaseItems[item_id].item_desc = item_desc;
            store('purchaseItems', JSON.stringify(purchaseItems));
            loadPurchaseItems();
        });
        $(document).on("change", '.RitemName', function () {
            ...
            var item_name = $(this).val(),
                item_id = row.attr('data-item-id');
            purchaseItems[item_id].item_name = item_name;
            store('purchaseItems', JSON.stringify(purchaseItems));
            loadPurchaseItems();
        });
        $(document).on("change", '.Rtax', function () {
            ...
            purchaseItems[item_id].taxname = taxname;
            store('purchaseItems', JSON.stringify(purchaseItems));
            loadPurchaseItems();
        });
        $('#PurchaseReset').click(function (e) {
    ...
    $(document).on('click', '.select_item', function (e) {
        ...
        $.ajax({
            type: "get",
            url: "<?= base_url()?>admin/global_controller/suggestions/" + saved_items_id,
            dataType: "json",
    ...
    function add_order_item(data) {
        ...
        if (purchaseItems[item_id]) {
            purchaseItems[item_id].qty = parseFloat(purchaseItems[item_id].qty) + 1;
        } else {
            purchaseItems[item_id] = data;
        }
        store('purchaseItems', JSON.stringify(purchaseItems));
        loadPurchaseItems();
        return true;
    ...
    function loadPurchaseItems() {
        ...
        if (get('purchaseItems')) {
            $("#purchaseTable").empty();
            purchaseItems = JSON.parse(get('purchaseItems'));
            $.each(purchaseItems, function () {
                var data = this;
                if (data.item_name) {
                    if (data.items_id) {
                    ...
                    var item_id = data.saved_items_id;
                    var product_id = data.saved_items_id, item_cost = data.cost_price, item_qty = data.qty, item_code = data.code,
                    ...
                    var amount = data.cost_price * data.qty;
                    ...
                    get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
                        ...
                        table_row += '</td>';
                        table_row += '<td class="rate"><input type="text" data-parsley-type="number"  onblur="calculate_total();" onchange="calculate_total();" name="items[' + item_key + '][unit_cost]" value="' + data.cost_price + '" class="form-control Rrate"></td>';
    ...
    function delete_item(row, itemid) {
        setTimeout(function () {
            ...
            if (purchaseItems[itemid]) {
                delete purchaseItems[itemid];
            }
            if (purchaseItems.hasOwnProperty(itemid)) {
            } else {
    ...
    function add_item_to_table(data, itemid, merge_invoice) {
        ...
        get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
            ...
            table_row += '</td>';
            table_row += '<td class="rate"><input type="text" data-parsley-type="number"  onblur="calculate_total();" onchange="calculate_total();" name="items[' + item_key + '][unit_cost]" value="' + data.cost_price + '" class="form-control"></td>';
            table_row += '<td class="taxrate">' + tax_dropdown + '</td>';
            table_row += '<td class="amount">' + amount + '</td>';
