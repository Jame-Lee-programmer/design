...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#estimate_item').bind('keypress', function (e) {
            if (e.keyCode == 13) {
                e.preventDefault();
        ...
        $(document).on("change", '.Rqty', function () {
            ...
            EstimateItems[item_id].qty = new_qty;
            store('EstimateItems', JSON.stringify(EstimateItems));
            loadEstimateItems();
        });
        ...
        $(document).on("change", '.Rrate', function () {
            ...
            var new_cost = $(this).val(),
                item_id = row.attr('data-item-id');
            EstimateItems[item_id].rate = new_cost;
            store('EstimateItems', JSON.stringify(EstimateItems));
            loadEstimateItems();
        });
        ...
        $(document).on("change", '.RitemDesc', function () {
            ...
            var item_desc = $(this).val(),
                item_id = row.attr('data-item-id');
            EstimateItems[item_id].item_desc = item_desc;
            store('EstimateItems', JSON.stringify(EstimateItems));
            loadEstimateItems();
        });
        $(document).on("change", '.RitemName', function () {
            ...
            var item_name = $(this).val(),
                item_id = row.attr('data-item-id');
            EstimateItems[item_id].item_name = item_name;
            store('EstimateItems', JSON.stringify(EstimateItems));
            loadEstimateItems();
        });
        $(document).on("change", '.Rtax', function () {
            ...
            EstimateItems[item_id].taxname = taxname;
            store('EstimateItems', JSON.stringify(EstimateItems));
            loadEstimateItems();
        });
        $('#EstimateReset').click(function (e) {
    ...
    function add_order_item(data, qty='') {
        ...
        if (EstimateItems[item_id]) {
            if (qty) {
                var qty = qty;
            } else {
                var qty = 1;
            }
            EstimateItems[item_id].qty = parseFloat(EstimateItems[item_id].qty) + parseFloat(qty);
        } else {
            EstimateItems[item_id] = data;
        }
        store('EstimateItems', JSON.stringify(EstimateItems));
        loadEstimateItems();
        return true;
    ...
    function loadEstimateItems() {
        ...
        if (get('EstimateItems')) {
            $("#EstimateTable").empty();
            EstimateItems = JSON.parse(get('EstimateItems'));
            $.each(EstimateItems, function () {
                var data = this;
                if (data.item_name) {
                    if (data.estimate_items_id) {
                        var old_items_id = data.estimate_items_id;
                    } else {
                        var old_items_id = '';
                    }
                    var item_id = data.saved_items_id;
                    ...
                    get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
                        // order input
                        table_row += '<input type="hidden" class="order" name="items[' + item_key + '][order]"><input type="hidden" name="items[' + item_key + '][saved_items_id]" value="' + data.new_itmes_id + '"><input type="hidden" name="items[' + item_key + '][estimate_items_id]" value="' + old_items_id + '">';
                        table_row += '</td>';
    ...
    function delete_item(row, itemid) {
        setTimeout(function () {
            ...
            if (EstimateItems[itemid]) {
                delete EstimateItems[itemid];
            }
            if (EstimateItems.hasOwnProperty(itemid)) {
            } else {
                localStorage.setItem('EstimateItems', JSON.stringify(EstimateItems));
                loadEstimateItems();
                return;
            }
            calculate_total();
