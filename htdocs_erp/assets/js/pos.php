...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#pos_item').bind('keypress', function (e) {
            if (e.keyCode == 13) {
                e.preventDefault();
        ...
        $(document).on("change", '.Rqty', function () {
            ...
            store('posItems', JSON.stringify(posItems));
            loadPosItems();
        });
        ...
        $(document).on("change", '.Rrate', function () {
            ...
            var new_cost = $(this).val(),
                item_id = row.attr('data-item-id');
            posItems[item_id].rate = new_cost;
            store('posItems', JSON.stringify(posItems));
            loadPosItems();
        });
        $(document).on("change", '.RitemDesc', function () {
            ...
            var item_desc = $(this).val(),
                item_id = row.attr('data-item-id');
            posItems[item_id].item_desc = item_desc;
            store('posItems', JSON.stringify(posItems));
            loadPosItems();
        });
        $(document).on("change", '.RitemName', function () {
            ...
            var item_name = $(this).val(),
                item_id = row.attr('data-item-id');
            posItems[item_id].item_name = item_name;
            store('posItems', JSON.stringify(posItems));
            loadPosItems();
        });
        $(document).on("change", '.Rtax', function () {
            ...
            posItems[item_id].taxname = taxname;
            store('posItems', JSON.stringify(posItems));
            loadPosItems();
        });
    ...
    function add_order_item(data) {
        ...
        if (posItems[item_id]) {
            posItems[item_id].qty = parseFloat(posItems[item_id].qty) + 1;
        } else {
            posItems[item_id] = data;
        }
        store('posItems', JSON.stringify(posItems));
        loadPosItems();
        return true;
    ...
    function loadPosItems() {
        ...
        if (get('posItems')) {
            $("#posTable").empty();
            posItems = JSON.parse(get('posItems'));
