...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("change", '.Rqty', function () {
            ...
            ProposalItems[item_id].qty = new_qty;
            store('ProposalItems', JSON.stringify(ProposalItems));
            loadProposalItems();
        });
        ...
        $(document).on("change", '.Rrate', function () {
            ...
            var new_cost = $(this).val(),
                item_id = row.attr('data-item-id');
            ProposalItems[item_id].rate = new_cost;
            store('ProposalItems', JSON.stringify(ProposalItems));
            loadProposalItems();
        });
        ...
        $(document).on("change", '.RitemDesc', function () {
            ...
            var item_desc = $(this).val(),
                item_id = row.attr('data-item-id');
            ProposalItems[item_id].item_desc = item_desc;
            store('ProposalItems', JSON.stringify(ProposalItems));
            loadProposalItems();
        });
        $(document).on("change", '.RitemName', function () {
            ...
            var item_name = $(this).val(),
                item_id = row.attr('data-item-id');
            ProposalItems[item_id].item_name = item_name;
            store('ProposalItems', JSON.stringify(ProposalItems));
            loadProposalItems();
        });
        $(document).on("change", '.Rtax', function () {
            ...
            ProposalItems[item_id].taxname = taxname;
            store('ProposalItems', JSON.stringify(ProposalItems));
            loadProposalItems();
        });
        $('#ProposalReset').click(function (e) {
    ...
    function add_order_item(data, qty='') {
        ...
        if (ProposalItems[item_id]) {
            if (qty) {
                var qty = qty;
            } else {
                var qty = 1;
            }
            ProposalItems[item_id].qty = parseFloat(ProposalItems[item_id].qty) + parseFloat(qty);
        } else {
            ProposalItems[item_id] = data;
        }
        store('ProposalItems', JSON.stringify(ProposalItems));
        loadProposalItems();
        return true;
    ...
    function loadProposalItems() {
        var merge_invoice = null;
        if (get('ProposalItems')) {
            $("#ProposalTable").empty();
            ProposalItems = JSON.parse(get('ProposalItems'));
            $.each(ProposalItems, function () {
                var data = this;
                if (data.item_name) {
                    if (data.proposals_items_id) {
                        var old_items_id = data.proposals_items_id;
                    } else {
                        var old_items_id = '';
                    }
                    var item_id = data.saved_items_id;
                    ...
                    get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
                        // order input
                        table_row += '<input type="hidden" class="order" name="items[' + item_key + '][order]"><input type="hidden" name="items[' + item_key + '][saved_items_id]" value="' + data.new_itmes_id + '"><input type="hidden" name="items[' + item_key + '][proposals_items_id]" value="' + old_items_id + '">';
                        table_row += '</td>';
...
//                    add_item_to_table(obj, 'undefined', _id);
    ...
    function delete_item(row, itemid) {
        setTimeout(function () {
            ...
            if (ProposalItems[itemid]) {
                delete ProposalItems[itemid];
            }
            if (ProposalItems.hasOwnProperty(itemid)) {
            } else {
                localStorage.setItem('ProposalItems', JSON.stringify(ProposalItems));
                loadProposalItems();
                return;
            }
            calculate_total();
