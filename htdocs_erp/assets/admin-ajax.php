...
//        alert('done');
    ...
    function buyPlan(pricing_id) {

        var base_url = '<?= base_url() ?>';
        var subscriptions_id = $.trim($("#subscriptions_id").val());
        var strURL = base_url + "admin/settings/buyPlan/" + subscriptions_id + '/' + pricing_id;
        var req = getXMLHTTP();
        if (req) {
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
                    ...
                    if (req.status == 200) {
                        var result = req.responseText;
                        $('#payment_method_' + pricing_id).empty();
                        $("#payment_method" + pricing_id).append(result);
                    } else {
                        alert("There was a problem while using XMLHTTP:\n" + req.statusText);
                    }
                }
