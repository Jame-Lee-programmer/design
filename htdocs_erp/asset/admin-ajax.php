<script language="javascript" type="text/javascript">
    ...
    function get_package_details(pricing_id) {
        if (pricing_id) {
            var strURL = "<?= base_url()?>admin/global_controller/get_package_details/" + pricing_id;
            var req = getXMLHTTP();
            if (req) {
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        ...
                        if (req.status == 200) {
                            ...
                            var msg = result.trim();
                            if (msg) {
                                document.getElementById('package_details').innerHTML = result;
                            }
                        } else {
                            alert("There was a problem while using XMLHTTP:\n" + req.statusText);
                        }
