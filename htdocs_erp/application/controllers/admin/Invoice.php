...
class Invoice extends Admin_Controller
{
    ...
    public function manage_invoice($action = NULL, $id = NULL, $item_id = NULL)
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($can_edit)) {
                ...
                if (empty($data['invoice_info'])) {
                    ...
                    $companies_id = $data['invoice_info']->companies_id;
                }
        ...
        if ($action == 'create_invoice') {
            ...
            $this->session->set_userdata($edit_invoice);
        } else {
            $data['active'] = 1;
        }
        // get all client
...
//            $invoice_items[$i]->new_itmes_id = $item->saved_items_id;
    ...
    public function save_invoice($id = NULL)
    {
        ...
        if (!empty($pos)) {
            ...
            $this->session->set_userdata($remove_pos);
            $permission = 'everyone';
        ...
        if (!empty($items_data)) {
            ...
            foreach ($items_data as $items) {
                ...
                if (!empty($qty_calculation) && $qty_calculation == 'Yes') {
                    if (!empty($items['saved_items_id']) && $items['saved_items_id'] != 'undefined' && empty($save_as_draft)) {
        ...
        if (!empty($Paid)) {
            $this->get_payemnt($invoice_id, true);
        }
    ...
    function recurring_invoice($id = NULL)
    {
        ...
        if (!empty($id)) {
            ...
            if (empty($data['invoice_info'])) {
                set_message('error', lang('there_in_no_value'));
                redirect('admin/invoice/recurring_invoice');
            }
            $data['active'] = 2;
        } else {
            $data['active'] = 1;
    ...
    function stop_recurring($invoices_id)
    {
        $update_recur = array(
            ...
            'recur_next_date' => '0000-00-00 00:00'
        );
        $this->invoice_model->_table_name = 'tbl_invoices';
        $this->invoice_model->_primary_key = 'invoices_id';
...
//            redirect($_SERVER['HTTP_REFERER']);
    ...
    public
    function get_payemnt($invoices_id, $pos = null)
    {
        $edited = can_action('13', 'edited');
        $can_edit = $this->invoice_model->can_action('tbl_invoices', 'edit', array('invoices_id' => $invoices_id));
        if (!empty($can_edit) && !empty($edited)) {
                    ...
                    if ($save_into_account == 'on') {
                        ...
                        if (!empty($account_id)) {
                            ...
                            $tr_data = array(
                                ...
                                'paid_by' => $inv_info->client_id,
                                'payment_methods_id' => (!empty($pos) ? '-' : $this->input->post('payment_methods_id', TRUE)),
                                'reference' => $trans_id,
    ...
    public function reminder($module, $module_id, $id = null)
    {
        ...
        if ($this->input->post()) {
            ...
            $data['all_reminder'] = get_result('tbl_reminders', array('module' => $module, 'module_id' => $module_id));
