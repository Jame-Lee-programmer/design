...
class Global_Controller extends MY_Controller
{
        ...
        if (!empty($rows)) {
            foreach ($rows as $row) {
                $html .= '<div class="col-xs-2 p0 m0" style="max-height:108px;overflow: hidden"> <a class="' . $class . ' btn p0" data-item-id="' . $row->saved_items_id . '">
                                            <img class="round" src="' . product_image($row->saved_items_id) . '" style="height: 70px;width: 70%"> <div class="text-xs-center text"> <small style="white-space: pre-wrap;">' . $row->item_name . ' ' . $row->code . '</small></div></a></div>';
            }
            echo $html;
        }
        exit();
    ...
    function json_get_department($companies_id)
    {
        ...
        if (!empty($data['all_dept_info'])) {
            foreach ($data['all_dept_info'] as $v_dept_info) {
                $all_department_info[] = $this->job_circular_model->get_add_department_by_id($v_dept_info->departments_id);
            }
        }
    ...
    public
    function get_milestone_by_project_id($project_id)
    {
    ...
    function get_item_name_by_id($stock_sub_category_id)
    {
        ...
        if (!empty($stock_info)) {
            foreach ($stock_info as $v_stock_info) {
                $items_info = itemsInfo($v_stock_info->saved_items_id);
                $HTML .= "<option value='" . $v_stock_info->stock_id . "'>" . $items_info->item_name . "</option>";
            }
        }
        echo $HTML;
        exit();
...
//            }
    ...
    function updatePackage()
    {
        ...
        $data['subview'] = $this->load->view('admin/settings/updatePackage', $data, TRUE);
        $this->load->view('admin/_layout_open', $data); //page load
    }
...
//        $this->load->model('settings_model');
        ...
        $data['currency_wise_price'] = $this->get_currencywise_price(true);
        $data['subview'] = $this->load->view('admin/settings/checkoutPayment', $data, TRUE);
...
//        if ($this->input->is_ajax_request()) {
        ...
        $data['set_merge_info'] = $this->load->view('admin/settings/checkout_payment', $_data, true);
...
//        }
    ...
    function active_subscription()
    {
        ...
        if (!$payment_method) {
            ...
            $this->payments_model->process_payment($this->input->post());
        } else {
            $type = 'error';
            $message = lang('please_select_payment_method');
            set_message($type, $message);
