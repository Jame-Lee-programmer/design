...
class Departments extends Admin_Controller
{
    ...
    public function index($id = NULL, $des_id = null)
    {
        ...
        if ($id) { // retrive data from db by id
            ...
            $data['department_info'] = get_row('tbl_departments', array('departments_id' => $id));
            $data['designations_info'] = get_row('tbl_designations', array('designations_id' => $des_id));
    ...
    public function save_departments($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            if (!empty($companies_id)) {
                ...
                $departments_id = $this->department_model->save($ddata, $departments_id);
            }
            // input data
            ...
            $d_menu = menu_by_company(1);
            $c_menu = menu_by_company(2);
            if (!empty($menu['menu'])) {
                $dashboard = in_array($d_menu, $menu['menu']);
                $calendar = in_array($c_menu, $menu['menu']);
            }
            if (empty($dashboard)) {
                $mdata['menu_id'] = $d_menu;
                $mdata['designations_id'] = $desi_id;
                $this->department_model->save($mdata);
            }
            if (empty($calendar)) {
                $mdata['menu_id'] = $c_menu;
                $mdata['designations_id'] = $desi_id;
                $this->department_model->save($mdata);
            }
            ...
            if (!empty($menu['menu'])) {
                foreach ($menu['menu'] as $key => $v_menu) {
                    if ($v_menu != 'undefined') {
                        if ($v_menu != $d_menu || $v_menu != $c_menu) {
    ...
    public function details($id = null)
    {
        ...
        if (!empty($edited)) {
            $data['title'] = lang('departments');
            if (!empty($id)) {
                $data['designations_info'] = get_row('tbl_designations', array('designations_id' => $id));
                if (!empty($data['designations_info'])) {
                    $data['departments_info'] = get_row('tbl_departments', array('departments_id' => $data['designations_info']->departments_id));
                }
    ...
    public function delete_department($id)
    {
        ...
        if (!empty($deleted)) {
            ...
            $get_designations_id = get_result('tbl_designations', $where);
    ...
    {
        $data['title'] = lang('edit') . ' ' . lang('departments');
        $data['users_info'] = get_result('tbl_account_details', array('designations_id' => $id));
        $data['designation_info'] = get_row('tbl_designations', array('designations_id' => $id));
