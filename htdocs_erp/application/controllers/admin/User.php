...
class User extends Admin_Controller
{
    ...
    public function user_list($action = NULL, $id = NULL)
    {
        ...
        if ($action == 'edit_user') {
            ...
            if (!empty($can_edit) || !empty($edited)) {
                ...
                if (!empty($data['login_info'])) {
                    $companies_id = $data['login_info']->companies_id;
                }
            }
        } else {
            $data['active'] = 1;
        ...
        $data['all_client_info'] = result_by_company('tbl_client');
        ...
        $data['all_designation_info'] = $this->user_model->all_designation($companies_id);
    ...
    public function save_user()
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            $login_data['super_admin'] = $this->input->post('super_admin', true);
            ...
            if (!empty($check_user) || !empty($check_email)) { // if input data already exist show error alert
                ...
                if (!empty($id)) {
                    ...
                    foreach ($all_client_menu as $v_client_menu) {
                        $client_data = $this->input->post($v_client_menu->label, true);
                        if (!empty($client_data)) {
                            $client_role_data['user_id'] = $id;
                            $client_role_data['menu_id'] = $client_data;
                ...
                if ($id == $user_id) {
                    ...
                    if (!empty($sbtn)) {
                        ...
                        if (!empty($result)) {
                            ...
                            $tickets_info = get_result('tbl_tickets');
                            ...
                            $leads_info = get_result('tbl_leads');
                            if (!empty($leads_info)) {
                            ...
                            // opportunity
                            $oppurtunity_info = get_result('tbl_opportunities');
                            ...
                            // project
                            $project_info = get_result('tbl_project');
                            ...
                            // tasks
                            $taskss_info = get_result('tbl_task');
                            if (!empty($taskss_info)) {
                                foreach ($taskss_info as $v_taskss) {
                                    if (!empty($v_taskss->permission) && $v_taskss->permission != 'all') {
                                        $allowad_user = json_decode($v_taskss->permission);
                            ...
                            // tbl_invoices
                            $invoices_info = get_result('tbl_invoices');
                            ...
                            // tbl_estimates
                            $estimate_info = get_result('tbl_estimates');
                            ...
                            // 	tbl_tax_rates
                            $tax_rate_info = get_result('tbl_tax_rates');
                            ...
                            $transactions_info = get_result('tbl_transactions');
                            ...
                            $transfer_info = get_result('tbl_transfer');
...
// crud for sidebar todo list
    ...
    public function reset_password($id)
    {
        if ($this->session->userdata('user_type') == 1) {
            ...
            $my_password = $this->input->post('my_password', true);
            if (!empty($new_password)) {

                $email = $this->session->userdata('email');
                $user_info = $this->db->where('user_id', $id)->get('tbl_users')->row();
                $my_info = $this->db->where('user_id', $this->session->userdata('user_id'))->get('tbl_users')->row();
                $my_password = $this->user_model->hash($my_password);
                if ($my_info->password == $my_password) {
                    $where = array('user_id' => $id);
