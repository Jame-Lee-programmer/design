    ...
    public function elfinder_init()
    {
        ...
        if (!empty($admin)) {
            ...
            if (!empty($companies_id)) {
                $company_info = get_row('tbl_companies', array('companies_id' => $companies_id));
                if (!empty($company_info)) {
                    $name = $company_info->name;
                } else {
                    $name = '-';
                }
                $company = slug_it($name);
            } else {
                $company = null;
            }
            $path = set_realpath('filemanager/' . $sub_name . $company);
            if (!is_dir($path)) {
                mkdir($path);
            }
...
//                'pattern' => '/.(' . $company . '+)/', // Prevent deleting/renaming folder
                'read' => true,
                'write' => true,
                'locked' => true
            ));
            $root_options['path'] = $path;
            $root_options['URL'] = site_url('filemanager/' . $sub_name . $company) . '/';
            ...
            if (!empty($companies_id)) {
                ...
                $company = slug_it($name);
                $realpath = 'filemanager/' . $sub_name . $company;
            } else {
                $realpath = 'filemanager/' . $sub_name;
            }

            $path = set_realpath($realpath . $user->media_path_slug);
            ...
            $root_options['path'] = $path;
            $root_options['URL'] = site_url($realpath . $user->media_path_slug) . '/';
        }
        $opts = array(
            'roots' => array(
                $root_options
