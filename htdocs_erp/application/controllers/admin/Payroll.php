...
// massage for user
    ...
    public function manage_salary_details($departments_id = NULL)
    {
        ...
        $data['all_department_info'] = by_company('tbl_departments', 'departments_id', null, $data['companies_id']);
        ...
        if (!empty($flag) || !empty($departments_id)) { // check employee id is empty or not
            ...
            $data['hourly_grade'] = get_result('tbl_hourly_rate');
            // get all salary Template info
            $data['salary_grade'] = get_result('tbl_salary_template');
        }
...
// get salary deduction info by salary template id
    ...
    public function set_salary_status($status, $id)
    {
        ...
        $activities = array(
            ...
            'icon' => 'cc-mastercard',
            'link' => 'admin/payroll/view_advance_salary/' . $id,
            'value1' => $profile_info->fullname . ' ' . lang('request_date') . display_datetime($advance_salary_info->request_date),
        ...
        if ($user_id != 0 && !empty($payment_month)) {
            ...
            if (!empty($check_existing_payment)) {
                foreach ($check_existing_payment as $key => $v_paymented_id) {
                    ...
                    $salary_payment_history = get_result('tbl_salary_payment_details', array('salary_payment_id' => $salary_payment_id));
