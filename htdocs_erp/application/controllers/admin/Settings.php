...
class Settings extends Admin_Controller
{
    ...
    public function save_system()
    {
        $input_data = $this->settings_model->array_from_post(array('default_language', 'locale',
            ...
            'enable_languages', 'allow_client_registration', 'allow_client_project', 'allow_sub_tasks', 'currency_position', 'money_format', 'google_api_key',
        ...
        $this->db->where(array('companies_id' => $companies_id, 'config_key' => 'date_picker_format'))->update('tbl_config', array("companies_id" => $companies_id, "value" => $picker));
        $this->db->where(array('companies_id' => $companies_id, 'config_key' => 'date_php_format'))->update('tbl_config', array("companies_id" => $companies_id, "value" => $phptime));
    ...
    public function save_payments($payment)
    {
        if ($payment == 'paypal') {
            ...
            $input_data = $this->settings_model->array_from_post(array('aim_api_login_id', 'aim_authorize_transaction_key', 'aim_authorize_status'));
    ...
    public function update_email()
    {
        ...
        if (!empty($smtp_pass)) {
            ...
            $exists = $this->db->where($where)->get('tbl_config');
            if ($exists->num_rows() == 0) {
                $this->db->insert('tbl_config', array("companies_id" => $companies_id, "config_key" => 'smtp_pass', "value" => $smtp_pass));
            }
        }
...
//            }
    ...
    public function purchase()
    {
        ...
        $data['title'] = lang('purchase_settings'); //Page title
        $can_do = can_do(118);
        if (!empty($can_do)) {
            $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
        } else {
    ...
    {
        $input_data = $this->settings_model->array_from_post(array('purchase_prefix', 'purchase_start_no', 'show_purchase_tax', 'increment_purchase_number', 'purchase_terms', 'purchase_footer'));
        $companies_id = $this->session->userdata('companies_id');
        if (empty($companies_id)) {
            $companies_id = null;
        }
        ...
        // messages for user
        $type = "success";
        $message = lang('save_purchase_settings');
        set_message($type, $message);
    ...
    public function lead_status($action = NULL, $id = NULL)
    {
        ...
        $data['all_lead_status'] = get_result('tbl_lead_status');
    ...
    public function customer_group($action = NULL, $id = NULL)
    {
        ...
        $data['all_customer_group'] = get_order_by('tbl_customer_group', array('type' => 'client'));
    ...
    public function manufacturer($action = NULL, $id = NULL)
    {
        ...
        if ($action == 'edit_manufacturer') {
            $data['active'] = 2;
            if (!empty($id) && !empty($edited)) {
                $data['manufacturer_info'] = $this->settings_model->check_by(array('manufacturer_id' => $id), 'tbl_manufacturer');
            }
        } else {
            $data['active'] = 1;
        }
        $data['page'] = lang('settings');
        $data['sub_active'] = lang('manufacturer');
        if ($action == 'update_manufacturer') {
            if (!empty($created) || !empty($edited)) {
                $this->settings_model->_table_name = 'tbl_manufacturer';
                $this->settings_model->_primary_key = 'manufacturer_id';
                ...
                // check whether this input data already exist or not
                $check_category = $this->settings_model->check_update('tbl_manufacturer', $where, $manufacturer_id);
                if (!empty($check_category)) { // if input data already exist show error alert
                    // massage for user
                    $type = 'error';
            ...
            redirect('admin/settings/manufacturer');
        } else {
            $data['title'] = lang('manufacturer'); //Page title
            $data['load_setting'] = 'manufacturer';
        }
        $data['all_manufacturer'] = get_order_by('tbl_manufacturer');
    ...
    public function delete_manufacturer($id)
    {
        ...
        if (!empty($deleted)) {
            $manufacturer = $this->settings_model->check_by(array('manufacturer_id' => $id), 'tbl_manufacturer');
            $activity = array(
                'user' => $this->session->userdata('user_id'),
                'module' => 'settings',
                'module_field_id' => $id,
                'activity' => ('activity_delete_a_manufacturer'),
                'value1' => $manufacturer->manufacturer,
            );
            $this->settings_model->_table_name = 'tbl_activities';
            $this->settings_model->_primary_key = 'activities_id';
            $this->settings_model->save($activity);
    ...
    public function clear_activities()
    {
        ...
        $activity = array(
            ...
            'value1' => lang('all_activity') . ' ' . date('Y-m-d H:i')
        );
        $this->settings_model->_table_name = 'tbl_activities';
        $this->settings_model->_primary_key = 'activities_id';
        $this->settings_model->save($activity);
...
//        exit();
    ...
    public function save_working_days()
    {
        ...
        if (!empty($office_time)) {
            foreach ($office_time as $key => $value) {
                ...
                $this->db->where($where)->update('tbl_config', $office_data);
                $exists = $this->db->where($where)->get('tbl_config');
                if ($exists->num_rows() == 0) {
                    $this->db->insert('tbl_config', array("companies_id" => $companies_id, "config_key" => $key, "value" => $value));
                }
    ...
    public function all_active_menu()
    {
        ...
        $user_menu = get_menu(array('status' => 1, 'companies_id' => $companies_id), $companies_id);
        $menu = array(
            'items' => array(),
            'parents' => array()
        );
    ...
    public function all_inactive_menu()
    {
        ...
        $user_menu = get_menu(array('status' => 0, 'companies_id' => $companies_id), $companies_id);
        $menu = array(
            'items' => array(),
            'parents' => array()
        );
    ...
    public function manage_status($status)
    {
        ...
        if ($this->input->post()) {
            ...
            $data['all_status'] = get_result('tbl_' . $status);
            $data['status'] = $status;
    ...
    public function upgradePlan($id, $companies_id = null)
    {
    ...
    public function manually_active($pricing_id, $subscriptions_id, $companies_id = null)
    {
        ...
        if (!empty($super_admin)) {
            ...
            if (!empty($companies_id)) {
                $pricing_info = get_row('tbl_frontend_pricing', array('id' => $pricing_id));
                if (empty($pricing_info)) {
                    $type = 'error';
                    $message = lang('sorry_this_pricing_plan_closed');
                } else {
                    $sub_data = array(
                        ...
                        'end_date' => calculate_plan_end_date($pricing_id),
                        'currency' => config_item('default_currency'),
                        'payment_method' => 'Manually Active',
                    ...
                    if (!empty($subscriptions_id) && $subscriptions_id != 0) {
                        $this->settings_model->save_data($sub_data, $subscriptions_id);
                    } else {
                        $this->settings_model->save_data($sub_data);
                    }
                    $type = 'success';
                    $message = lang('congrass_successfuly_active_plan', company_name($companies_id));
    ...
    public
    function buyPlan($subscriptions_id = NULL, $pricing_id = null)
    {
        $sub_info = get_row('tbl_subscriptions', array('subscriptions_id' => $subscriptions_id));
        $plan_info = plan_info($pricing_id);
        $HTML = null;
        $HTML .= '<ul class="nav" style="text-align: left" id="payPlan">';
        if ($plan_info->allow_paypal == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/paypal/payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('paypal') . "</a></li>";
        }
        if ($plan_info->allow_2checkout == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/checkout/payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('2checkout') . "</a></li>";
        }
        if ($plan_info->allow_stripe == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/stripe/payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('stripe') . "</a></li>";
        }
        if ($plan_info->allow_authorize == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/authorize/payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('authorize') . "</a></li>";
        }
        if ($plan_info->allow_ccavenue == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/ccavenue/payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('ccavenue') . "</a></li>";
        }
        if ($plan_info->allow_braintree == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/braintree/payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('braintree') . "</a></li>";
        }
        if ($plan_info->allow_mollie == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/mollie/pay//payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('mollie') . "</a></li>";
        }
        if ($plan_info->allow_payumoney == 'Yes') {
            $HTML .= "<li><a data-toggle='modal' data-target='#myModal' href='" . base_url('payment/payumoney/payPlan/' . $plan_info->id . '/' . $sub_info->subscriptions_id) . "'>" . lang('PayUmoney') . "</a></li>";
        }
        $HTML .= '</ul>';
        echo $HTML;
        exit();
    ...
    {
        $data['page'] = lang('settings');
        $data['title'] = lang('server') . ' ' . lang('settings');

        $data['load_setting'] = 'server';
        $can_do = super_admin();
        if (!empty($can_do)) {
            $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
        } else {
    ...
    public function save_server()
    {
        ...
        if (!empty($can_do)) {
            ...
            if ($saas_server == 'cpanel') {
                $input_data = $this->settings_model->array_from_post(array('saas_server', 'saas_cpanel_host',
                ...
                $input_data = $this->settings_model->array_from_post(array('saas_server', 'saas_plesk_host', 'saas_plesk_username'
                , 'saas_plesk_webspace_id'));
                ...
                $input_data = $this->settings_model->array_from_post(array('saas_server'));
            }
