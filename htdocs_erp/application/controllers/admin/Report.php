...
class Report extends Admin_Controller
{
    ...
    {// this function is to create get monthy recap report
        ...
        for ($i = 1; $i <= $num; $i++) {
            ...
            $date = $date . '-' . $i;
            $transaction_report[$i] = get_order_by('tbl_transactions', array('date' => $date));
        }
    ...
    {// this function is to create get monthy recap report
        ...
        for ($i = 1; $i <= 12; $i++) { // query for months
            ...
            if (!empty($tickets)) {
                ...
                $get_result[$i] = get_result('tbl_tickets', $where);
            } else {
                $where = array('created_time >=' => $start_date, 'created_time <=' => $end_date);
                $get_result[$i] = get_result('tbl_bug', $where);
            }
        }
    ...
    function sales_report_pdf($filterBy = null, $status, $start_date = null, $end_date = null)
    {
        ...
        if (!empty($filterBy)) {
            ...
            if ($filterBy == 'invoices' || $filterBy == 'invoice_by_client') {
                if (!empty($start_date)) {
                    $data['range'] = array($start_date, $end_date);
                    $date = lang('FROM') . ' ' . display_datetime($start_date) . ' ' . lang('TO') . ' ' . display_datetime($end_date);
                } else {
                    $start_date = null;
                    $end_date = null;
                    $date = null;
