...
class Stock extends Admin_Controller
{
    ...
    public function stock_category($id = null, $sub_id = null)
    {
        ...
        if ($id) { // retrive data from db by id
            ...
            if (!empty($edited)) {
                $data['category_info'] = get_row('tbl_stock_category', array('stock_category_id' => $id));
                $data['sub_category_info'] = get_row('tbl_stock_sub_category', array('stock_sub_category_id' => $sub_id));
            }
    ...
    public function items_details_stock_category($stock_sub_category_id)
    {
        $data['title'] = lang('stock_category');
        $data['sub_category'] = get_row('tbl_stock_sub_category', array('stock_sub_category_id', $stock_sub_category_id));
        $data['items_details'] = $this->stock_model->get_all_stock_info($stock_sub_category_id);
        $data['subview'] = $this->load->view('admin/stock/items_details_stock_category', $data, FALSE);
    ...
    public function save_stock_category($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            if (!empty($companies_id)) {
                $ddata['companies_id'] = $companies_id;
                $stock_category_id = $this->stock_model->save($ddata, $stock_category_id);
            }
            // input data
    ...
    public function edit_stock_category($id)
    {
        ...
        if (!empty($edited)) {
            ...
            $data['stock_category_info'] = get_row('tbl_stock_category', array('stock_category_id' => $id));
    ...
    public function stock_list($id = null)
    {
        ...
        if ($id) { // retrive data from db by id
            ...
            if (!empty($edited)) {
                $data['stock_info'] = get_row('tbl_stock', array('stock_id' => $id));
                if (!empty($data['stock_info'])) {
                    $data['items_stock_info'] = $this->stock_model->get_all_stock_info($data['stock_info']->stock_sub_category_id);
                }
                if (empty($data['stock_info'])) {
                    $type = "error";
                    $message = lang('no_record_found');
        ...
        foreach ($all_stock_info as $v_stock_info) {
            $data['all_stock_info'][$v_stock_info->stock_category_id][$v_stock_info->stock_sub_category][] = $v_stock_info;
        }
    ...
    public function save_stock($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            $data = $this->stock_model->array_from_post(array('stock_sub_category_id', 'saved_items_id')); //input post
            $where = array('stock_sub_category_id' => $data['stock_sub_category_id'], 'saved_items_id' => $data['saved_items_id']);
            ...
            $activity = array(
                ...
                'module_field_id' => $id,
                'activity' => $actv,
                'value1' => itemsName($data['saved_items_id'])
            );
            $this->stock_model->_table_name = 'tbl_activities';
            $this->stock_model->_primary_key = 'activities_id';
            $this->stock_model->save($activity);
    ...
    function delete_stock($stock_id)
    {
        ...
        if (!empty($deleted)) {
            ...
            $check_existing_stock_id = get_row('tbl_assign_item', array('stock_id' => $stock_id));
            if (!empty($check_existing_stock_id)) {
                ...
                // else do not delete
                $stock_info = get_row('tbl_stock', array('stock_id' => $stock_id));
                ...
                $activity = array(
                    ...
                    'value1' => itemsName($stock_info->saved_items_id)
                );
                $this->stock_model->_table_name = 'tbl_activities';
                $this->stock_model->_primary_key = 'activities_id';
                $this->stock_model->save($activity);
    ...
    function stock_history($sub_category_id = NULL)
    {
        ...
        if (!empty($flag) || !empty($sub_category_id)) {
            ...
            foreach ($get_stock_id as $v_stock_id) {
                $items_info = itemsInfo($v_stock_id->saved_items_id);
                $data['item_history_info'][$v_stock_id->stock_sub_category][$items_info->item_name] = $this->stock_model->get_item_history_by_id($v_stock_id->stock_id);
            }
        }
...
//        redirect('admin/stock/stock_history/' . $sub_category_id);
    ...
    function assign_stock($id = NULL)
    {
        ...
        if (!empty($id)) {
            if (!empty($edited)) {
                ...
                $data['assign_item'] = get_row('tbl_assign_item', array('assign_item_id' => $id));
                // get sub category id by stock id
                $data['stock_info'] = get_row('tbl_stock', array('stock_id' => $data['assign_item']->stock_id));
    ...
    function json_get_stock($companies_id)
    {
        $all_category_info = array();
        // retrive all data from tbl_stock_category table
        $all_cate_info = by_company('tbl_stock_category', 'stock_category_id', null, $companies_id);
        // get all category info and get_sub_category info
        foreach ($all_cate_info as $v_cate_info) {
            $all_category_info[$v_cate_info->stock_category] = $this->stock_model->get_sub_category_by_id($v_cate_info->stock_category_id);
        }
        echo json_encode($all_category_info);
        exit();
    }
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            if (empty($check_enough_stock)) {
                ...
                $activity = array(
                    ...
                    'value1' => itemsName($total_stock->saved_items_id)
                );
                $this->stock_model->_table_name = 'tbl_activities';
                $this->stock_model->_primary_key = 'activities_id';
                $this->stock_model->save($activity);
    ...
    function assign_report_pdf($start_date, $end_date)
    {
        ...
        pdf_create($viewfile, lang('stock') . ' ' . lang('report') . ' ' . display_datetime($start_date) . ' To ' . display_datetime($end_date));
    }
