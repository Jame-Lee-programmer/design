...
<?php
$currency = $currency = get_old_data('tbl_currencies', array('code' => config_item('default_currency')));
...
<div class="modal-body pt0 wrap-modal wrap">
    ...
    <div class="col-sm-8 row">
        <div class="panel panel-custom">
            ...
            <div class="panel-body">
                ...
                <span>No taxes, or hidden fees included.<br> We accept Visa, Master Card, American Express,Strip,2Checkout,PayUmoney,Mollie, or PayPal.</span>
          ...
          <path class='deco-layer deco-layer--1'
                ...
                fill='#FFFFFF' opacity='0.6'></path>
                                ...
                                <div class='packaging-package'><span
                                            class='packaging-currency'><?= $currency->symbol ?></span><?= $package_info->amount ?>
                                    <span
                                            class='packaging-period'>/ <?= 0 ?></span>
                                </div>
    ...
    <div class="col-sm-4">
        <div class="panel panel-custom"
             style="box-shadow: 0 0px 0px 0 rgba(0,0,0,.15);overflow: hidden">
            ...
            <div class="panel-body">
                <?php if ($package_info->allow_paypal == 'Yes') {
                    ?>
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb "
                       href="<?= base_url() ?>payment/paypal/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('paypal') ?>"><img style="width: 80px;height: 50px"
                                                          src="<?= base_url('asset/images/payment_logo/paypal.png') ?>">
                    </a>
                    <?php
                }
                if ($package_info->allow_2checkout == 'Yes') {
                    ...
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb"
                       href="<?= base_url() ?>payment/checkout/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('2checkout') ?>">
                        <img style="width: 80px;height: 50px"
                             src="<?= base_url('asset/images/payment_logo/2checkout.jpg') ?>">
                    </a>
                <?php }
                if ($package_info->allow_stripe == 'Yes') {
                    ...
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb"
                       title="<?= lang('stripe') ?>"
                       href="<?= base_url() ?>payment/stripe/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>">
                        <img style="width: 80px;height: 50px"
                             src="<?= base_url('asset/images/payment_logo/stripe.jpg') ?>">
                    </a>
                <?php }
                if ($package_info->allow_authorize == 'Yes') { ?>
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb"
                       href="<?= base_url() ?>payment/authorize/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('authorize') ?>"><img style="width: 80px;height: 50px"
                                                             src="<?= base_url('asset/images/payment_logo/Authorizenet.png') ?>"></a>
                <?php }
                if ($package_info->allow_ccavenue == 'Yes') { ?>
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb"
                       href="<?= base_url() ?>payment/ccavenue/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('ccavenue') ?>"><img style="width: 80px;height: 50px"
                                                            src="<?= base_url('asset/images/payment_logo/CCAvenue.jpg') ?>"></a>
                <?php }
                if ($package_info->allow_braintree == 'Yes') { ?>
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb"
                       href="<?= base_url() ?>payment/braintree/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('braintree') ?>"><img style="width: 80px;height: 50px"
                                                             src="<?= base_url('asset/images/payment_logo/Braintree.png') ?>"></a>
                <?php }
                if ($package_info->allow_mollie == 'Yes') { ?>
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb"
                       href="<?= base_url() ?>payment/mollie/pay/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('mollie') ?>"><img style="width: 80px;height: 50px"
                                                          src="<?= base_url('asset/images/payment_logo/ideal_mollie.png') ?>"></a>
                <?php }
                if ($package_info->allow_payumoney == 'Yes') { ?>
                    <a data-toggle='modal' data-target='#myModal' class="pull-left pb"
                       href="<?= base_url() ?>payment/payumoney/payPlan/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('PayUmoney') ?>"><img style="width: 80px;height: 50px"
                                                             src="<?= base_url('asset/images/payment_logo/payumoney.jpg') ?>"></a>
                <?php }
                $super_admin = super_admin();
                if (!empty($super_admin)) {
                    ?>
                    <a onclick="return confirm('<?= lang('alert_manually_payment') ?>');"
                       class="pull-left pb"
                       href="<?= base_url() ?>admin/settings/manually_active/<?= $package_info->id . '/' . $sub_info->subscriptions_id ?>"
                       title="<?= lang('manually_active') ?>"><img style="width: 80px;height: 50px"
                                                                   src="<?= base_url('asset/images/payment_logo/cash_payment.png') ?>"></a>
                <?php } ?>
