<style type="text/css">
    ...
    .pricing-palden li.pricing-feature {
...
<span class="set_price btn btn-xs custom-bg active by_type" result="by_type"
      type="monthly"><?= lang('monthly') ?></span>
                                       <span class="set_price btn btn-xs btn-primary by_type" type="annually"
...
<div class="pull-right currency_type">
    <?php
    $all_currency = currency_list();
    if (!empty($all_currency)) {
        foreach ($all_currency as $v_currency) {
            $c_name = get_old_data('tbl_currencies', array('code' => $v_currency->currency));
            ?>
            <span currency="<?= $v_currency->currency ?>" result="by_currency"
                  class="set_price btn btn-xs currencywise_price <?= (config_item('default_currency') == $v_currency->currency ? 'custom-bg active' : 'btn-primary') ?>"><?= $c_name->name . '(' . $c_name->symbol . ')' ?></span>
        <?php }
    }
    ?>
</div>
...
<?php echo form_open(base_url('checkoutPayment'), array('class' => 'form-horizontal', 'id' => 'contact-form', 'enctype' => 'multipart/form-data', 'data-parsley-validate' => '', 'role' => 'form')); ?>

<input id="new_interval_type" name="interval_type" value="monthly" type="hidden">
<input name="front_end" value="1" type="hidden">
<input id="new_currency_type" name="currency_type" value="<?= config_item('default_currency') ?>" type="hidden">

<section class="price-section">
    <div class='pricing pricing-palden'>
        <?php
        $all_pricing = get_old_order_by('tbl_frontend_pricing', array('status' => 1), 'sort', true);
        ...
        if (!empty($all_pricing)) {
            foreach ($all_pricing as $pricing) {
                ?>
                <div
                    class='pricing-item table_list_<?= $pricing->id ?>'>
                    <?php if (!empty($sub_info->pricing_id) && $pricing->id == $sub_info->pricing_id) { ?>
                        <div class="ribbon-wrapper-red">
                            <div class="ribbon-red"><?= lang('selected') ?></div>
                        </div>
                    <?php } ?>

                    <div class='pricing-deco custom-bg'>
...
<path class='deco-layer deco-layer--1'
      d='M30.913,43.944c0,0,42.911-34.464,87.51-0.191c77.31,35.0,113.304-1.952,146.638-4.729&#x000A;	c48.654-4.056,69.94,16.218,69.94,16.218v54.396H30.913V43.944z'
      fill='#FFFFFF' opacity='0.6'></path>
                            <path class='deco-layer deco-layer--2'
                                  d='M-35.667,44.628c0,0,42.91-34.463,87.51-0.191c77.31,35.141,113.304-1.952,146.639-4.729&#x000A;	c48.653-4.055,69.939,16.218,69.939,16.218v54.396H-35.667V44.628z'
                                  fill='#FFFFFF' opacity='0.6'></path>
                        ...
                        <div class='pricing-price'><span
                                class='pricing-currency'><?= $currency->symbol ?></span>
                            <span class="type_amount_<?= $pricing->id ?>"><?= $pricing->amount ?></span>
                                                                                                                                                                         <span
                                                                                                                                                                             class='pricing-period'></span>
                        </div>
                        <h3 class='pricing-title'><?= $pricing->name ?></h3>
                    </div>
                    <ul class='pricing-feature-list'>
                        <li class='pricing-feature'><?= pricing_format_admin_YN($pricing->multi_branch, lang('multi_branch')) ?></li>
                        <li class='pricing-feature'><?= pricing_format_admin($pricing->employee_no, lang('employee')) ?></li>
                        <li class='pricing-feature'><?= pricing_format_admin($pricing->disk_space, lang('disk_space')) ?></li>
                        <li class='pricing-feature'><?= pricing_format_admin($pricing->trial_period, lang('days') . ' ' . lang('trail_period')) ?></li>
                        <li class='pricing-feature'><?= pricing_format_admin($pricing->client_no, lang('client')) ?></li>
                        <li class='pricing-feature'><?= pricing_format_admin($pricing->project_no, lang('project')) ?></li>
                        <li class='pricing-feature'><?= pricing_format_admin($pricing->invoice_no, lang('invoice')) ?></li>
                        <li class='pricing-feature'><?= pricing_format_admin($pricing->leads, lang('leads')) ?></li>
                    </ul>
                    <a data-toggle='modal' data-target='#myModal'
                       href="<?= base_url('admin/global_controller/package_details/' . $pricing->id) ?>"
                       class="text-center"><?= lang('see') . ' ' . lang('details') ?></a>

                    <button type="submit" value="<?= $pricing->id ?>" name="pricing_id" style="margin-right: 25px;"
                            class="btn btn-purple ml mt-lg"><?= lang('buy') ?></button>
                </div>
            <?php }
        } ?>
    </div>
