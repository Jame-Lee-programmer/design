...
class Invoice_Model extends MY_Model
{
    ...
    public function invoice_payment($invoice)
    {
        ...
        $companies_id = $this->ci->session->userdata('companies_id');
        if (!empty($companies_id)) {
            if ($this->ci->db->field_exists('companies_id', 'tbl_payments')) {
                $this->ci->db->where('companies_id', $companies_id);
            }
        }
        $query = $this->ci->db->get('tbl_payments');
        if ($query->num_rows() > 0) {
            $row = $query->row();
            return $row->amount;
        }
        ...
        if (!empty($json)) {
            if (!empty($rows)) {
                foreach ($rows as $row) {
                    ...
                    $row->taxname = $this->get_invoice_item_taxes($row->items_id);;
    ...
    public function get_invoice_paid_amount($invoice_id)
    {
        ...
        if (!empty($companies_id)) {
            if ($this->db->field_exists('companies_id', 'tbl_payments')) {
    ...
    {
        $invoices = get_result('tbl_invoices');
        $cost[] = array();
    ...
    function client_outstanding($client_id, $project_id = null)
    {
        $due[] = array();
        if (!empty($project_id)) {
            $invoices_info = get_result('tbl_invoices', array('project_id' => $project_id));
        } else {
            $invoices_info = get_result('tbl_invoices', array('client_id' => $client_id));
        }
    ...
    {
        if (!empty($client_id)) {
            return get_result('tbl_invoices', array('recurring' => 'Yes', 'client_id' => $client_id));
        } else {
            return get_result('tbl_invoices', array('recurring' => 'Yes', 'invoices_id' => 0));
        }
    }
    ...
    {
        if (!empty($client_id)) {
            $all_invoice = get_result('tbl_invoices', array('recurring' => 'Yes', 'client_id' => $client_id));
        } else {
            $all_invoice = $this->get_permission('tbl_invoices');
        }
    ...
    {
        $all_invoice = get_result('tbl_payments');
        $all_method = get_result('tbl_payment_methods');
        if (!empty($all_invoice)) {
            $all_invoice = array_reverse($all_invoice);
            foreach ($all_invoice as $v_invoices) {
    ...
    {
        if (!empty($client_id)) {
            $all_payments = get_result('tbl_payments', array('paid_by' => $client_id));
        } else {
...
//            }
    ...
    public function paid_by_date($year, $month = null)
    {
        ...
        $this->_table_name = "tbl_payments"; //table name
        $this->_order_by = "payments_id";
        $payments = $this->get_by($where, FALSE);
...
//            }
    ...
    public function get_invoice_report($filterBy = null, $range = null)
    {
        if (!empty($filterBy) && is_numeric($filterBy)) {
            $invoice = get_result('tbl_invoices', array('client_id' => $filterBy));
        } else {
    ...
    public function get_payment_report($client_id = null, $range = null)
    {
        ...
        if (!empty($where)) {
            ...
            return get_result('tbl_payments');
        }
