...
//        $this->db->where('tbl_menu.status !=', 2);
    ...
    {
        $emp_payroll = get_row('tbl_employee_payroll', array('user_id' => $user_id));
        if (!empty($emp_payroll)) {
            if (!empty($emp_payroll->salary_template_id)) {
                $emp_salary = get_row('tbl_salary_template', array('salary_template_id' => $emp_payroll->salary_template_id));
                $basic_salary = $emp_salary->basic_salary;
            }
            if (!empty($emp_payroll->hourly_rate_id)) {
                $hourly_salary = get_row('tbl_hourly_rate', array('hourly_rate_id' => $emp_payroll->hourly_rate_id));
