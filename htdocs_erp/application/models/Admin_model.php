...
class Admin_Model extends MY_Model
{
    ...
    public function select_menu_by_uri($uriSegment)
    {
        ...
        $result = get_row($table, array('link' => $uriSegment));
        if (count($result)) {
    ...
    function get_transactions_list_by_date($type, $start_date, $end_date)
    {
        ...
        if (!empty($companies_id)) {
            if ($this->db->field_exists('companies_id', 'tbl_transactions')) {
    ...
    function get_goal_report_by_month($start_date, $end_date)
    {
        ...
        $companies_id = $this->session->userdata('companies_id');
        if (!empty($companies_id)) {
            if ($this->db->field_exists('companies_id', 'tbl_goal_tracking')) {
                $this->db->where('companies_id', $companies_id);
            }
        }
    ...
    public function get_taxes_dropdown($name, $taxname, $type = '', $item_id = '', $is_edit = false, $manual = false)
    {
        ...
        $select = '<select class="selectpicker display-block Rtax tax" data-width="100%" name="' . $name . '" multiple data-none-selected-text="' . lang('no_tax') . '">';
        foreach ($taxes as $tax) {
            $selected = '';
            if (is_array($taxname)) {
