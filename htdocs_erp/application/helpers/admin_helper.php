...
function super_admin_opt_th()
{
    ...
    if (!empty($super_admin) && $super_admin == 'Yes' || !empty($super_admin) && $super_admin == 'owner') {
        echo '<th>' . lang('companies') . '</th>';
    } else {
        return false;
    }
}

function super_admin_opt_td($companies_id = null)
{
    $CI =& get_instance();
    $super_admin = $CI->session->userdata('super_admin');
    if (!empty($super_admin) && $super_admin == 'Yes' || !empty($super_admin) && $super_admin == 'owner') {
...
function super_admin_inline($companies_id = null)
{
    ...
    $super_admin = $CI->session->userdata('super_admin');
    if (!empty($super_admin) && $super_admin == 'Yes' || !empty($super_admin) && $super_admin == 'owner') {
        echo '<td><select name="companies_id" class="form-control select_box" style="width:100%" required>
...
function super_admin_form($companies_id, $lcol = null, $incol = null)
{
    ...
    if (!empty($super_admin) && $super_admin == 'Yes' || !empty($super_admin) && $super_admin == 'owner') {
        ...
        $html .= '<div class="form-group">
                <label class="col-md-' . $lcol . ' control-label">' . lang('select') . ' ' . lang('companies') . '<span class="text-danger">*</span></label>
                <div class="col-lg-' . $incol . '">
                <select name="companies_id" class="form-control select_box" style="width:100%" required>
...
function super_admin_details($companies_id, $lcol = null, $incol = null)
{
    $CI =& get_instance();
    $html = null;
    $super_admin = $CI->session->userdata('super_admin');
    if (!empty($super_admin) && $super_admin == 'Yes' || !empty($super_admin) && $super_admin == 'owner') {
        ...
        $html .= '<div class="form-group">
                <label class="col-md-' . $lcol . '">' . lang('companies') . ': </label>
                <div class="col-md-' . $incol . '">' . $name . '</div>
                </div>';
        echo $html;
    } else {
        return false;
...
function super_admin_details_p($companies_id, $lcol = null, $mcol = null)
{
    ...
    if (!empty($super_admin) && $super_admin == 'Yes' || !empty($super_admin) && $super_admin == 'owner') {
        ...
        $html .= '<div class="' . $mcol . '">
                <label class="control-label col-md-' . $lcol . '">' . lang('companies') . ': </label>
...
{
    $subs_info = get_row('tbl_subscriptions', array('domain' => $domain, 'status !=' => 'pending'));
    if (!empty($subs_info)) {
...
{
    $subs_info = get_row('tbl_subscriptions', array('domain' => $domain));
    if (empty($subs_info)) {
        return false;
    } else {
        return true;
...
function getDescription($input_post, $str = null)
{
    ...
    if (!empty($plan_info)) {
        ...
        if (!empty($str)) {
            ...
            $plan_name = '<a data-toggle="modal" data-target="#myModal" href="' . base_url('admin/global_controller/subs_package_details/' . $plan_info->id) . '">' . lang('for') . ' ' . lang('plan') . ' :' . $plan_info->name . ' ' . display_money($input_data->total, $currency->symbol) . ' /' . $frequency . ' ' . '</a>';
        }
    } else {
        $plan_name = '-';
    }
