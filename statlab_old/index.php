<?php
	...
	$result = sqlsrv_query($conn, "SELECT * FROM documents where daterun > '".date_format($firstDate,"Y-m-d")."' and  daterun < '".date_format($endDate,"Y-m-d")."' and (clientID = '$userid' or patientID='$userid') and patientID !=0") or die("database error:". sqlsrv_error($conn));
	// var_dump($result,2);
?>	
<head>
...
<header style="width:100%; bottom: 0px; position:relative;">   
    <div style="background-color: #1b212b;padding: 0px 0 20px 0; ">
		...
		<a href="logout.php" style="color: #ffffff;float:right;margin-top:20px; margin-right:10%;font-size: 25px;line-height: 36px;font-weight: bold; font-family: ' Open Sans', sans-serif;">Log out</a>
    </div>
</header>
						...
						<tbody id="pdftable">							
							...
							if (sqlsrv_num_fields($result) > 0) {
								...
								while($row = sqlsrv_fetch_array($result,2)) {							
									echo "<tr id='rowid' data-row='".$row['id']."'>";
...
function showresults(){
	...
	$("#pdftable").remove();
	$.post("tableList.php",{firstdate: $("#d1").val(), lastdate: $("#d2").val()}, function(data, status){
		if(status == "success"){
