<?php
	...
	$resultarray = sqlsrv_query($conn, "SELECT * FROM users where (ProviderID = '$userid' or patientID = '$userid')") or die("database error:". sqlsrv_error($conn));
	$row = sqlsrv_fetch_array($resultarray,2);
	$result = sqlsrv_query($conn, "SELECT * FROM documents where daterun >= '".date_format($firstDate,"Y-m-d")."' and  daterun <= '".date_format($endDate,"Y-m-d")."' and ProviderID = '$userid'") or die("database error:". sqlsrv_error($conn));
...
function showresults(){
	...
	$("#pdftable").remove();
	$.post("tableList.php",{firstdate: $("#d1").val(), lastdate: $("#d2").val(), type: 2}, function(data, status){
		if(status == "success"){
